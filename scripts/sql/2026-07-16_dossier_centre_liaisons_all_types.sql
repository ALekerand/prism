-- PRISM : généraliser les liaisons dossier centre (Alpha → tous types)
-- Avant : ID_CENTRE référence alpha.ID_CENTRE
-- Après : ID_CENTRE référence centre.ID_CENTRE uniquement
--
-- IMPORTANT : avec ddl-auto=update, Hibernate peut AJOUTER la FK vers centre
-- SANS supprimer l'ancienne FK vers alpha → double contrainte → 409 Conflict
-- sur CEC/CP/SIE (ex. POST /api/materielalpha/sync).
-- Ce script droppe toute FK vers alpha, puis assure la FK centre.
--
-- À exécuter sur local ET staging/prod. Idempotent : rejouable.
